...
class DreamBoothTiDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(np.array(instance_image).shape)
        # print(torch.tensor(np.array(instance_image)[:,:,3]))
        instance_mask = (
            transforms.Resize(size=64)(
                torch.tensor(np.array(instance_image)[:, :, 3]).unsqueeze(0)
            )
            / 255.0
        )
        # print("Initial instance mask shape", instance_mask.shape)
        # print("Initial image shape", instance_image.size)
        example["instance_mask"] = instance_mask
        if not instance_image.mode == "RGB":
            instance_image = instance_image.convert("RGB")
        example["instance_images"] = self.image_transforms(instance_image)

        placeholder_string = self.instance_prompt
...
def run(args):
    ...
    def collate_fn(examples):
        ...
        # print("First example mask shape", examples[0]["instance_mask"])
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            loss = F.mse_loss(
                model_pred.float() * batch["mask"].float(),
                target.float() * batch["mask"].float(),
                reduction="mean",
            )
